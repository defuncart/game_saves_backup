...
void main() {
  generateSteamIcon(onBuildIcon: (size) => Theme(data: darkTheme, child: SteamAsset(size: size)));
...
class SteamAsset extends StatelessWidget {
  const SteamAsset({super.key, required this.size});
    ...
    return Container(
      ...
      child: Center(
        child: Row(
          ...
          children: [
            Icon(Icons.backup, color: context.colorScheme.primary, size: size.shortestSide * 0.6),
            if (size.shortestSide > 600) ...[
              const SizedBox(width: 128),
              Text(
                'Game\nSaves\nBackup',
                style: TextStyle(fontSize: size.shortestSide * 0.6 * 0.1825, color: context.colorScheme.primary),
              ),
            ],
          ],
        ),

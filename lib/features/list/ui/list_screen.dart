      ...
      floatingActionButton: FloatingActionButton.large(
        ...
        onPressed: () async {
          ...
          if (directoryPath != null) {
            ref.read(backupItemsProvider.notifier).add(path: directoryPath);
          }
        },
      ),
    );
...
class _BackupItemTile extends ConsumerStatefulWidget {
  const _BackupItemTile({required this.item});
    ...
    return ContextMenuRegion(
      ...
      onItemSelected: (item) {
        if (item.title == context.l10n.listScreenBackupItemOpen) {
          ...
          Clipboard.setData(ClipboardData(text: _item.path));
          ...
          child: ListTile(
            ...
            title: TextField(
              ...
              onEditingComplete: () {
                final text = _controller.text.trim();
                if (text.isNotEmpty) {
                  ref.read(backupItemsProvider.notifier).updateName(item: _item, folderName: text);
                  _focusNode.unfocus();
                  setState(() => _hasUnsavedChanges = false);
                }
              },
              focusNode: _focusNode,
              decoration: InputDecoration.collapsed(hintText: context.l10n.listScreenBackupItemNameHintText),
              // disable context menu to avoid interfering with native context menu
              contextMenuBuilder: (context, state) => const SizedBox.shrink(),
            ),
            trailing: IconButton(icon: const Icon(Icons.folder), onPressed: onOpen),
            subtitle: Column(
              ...
              children: [
                ...
                if (_hasUnsavedChanges)
                  Text(
                    ...
                    style: context.textTheme.labelSmall?.apply(color: context.colorScheme.error),
                  ),
              ],
            ),
          ),
